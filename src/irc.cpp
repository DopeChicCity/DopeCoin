...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        CService addrConnect("92.51.132.40", 6667); // irc.dopecoin.com

        CService addrIRC("irc.dopecoin.com", 6667, true);
        if (addrIRC.IsValid())
            addrConnect = addrIRC;
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #dopecoinTEST0\r");
            Send(hSocket, "WHO #dopecoinTEST0\r");
        } else {
